...
public class QualificationMaxSpeedService : IQualificationMaxSpeedService
{
    private readonly IMapper _mapper;
    private readonly IRepositoryManager _repositoryManager;

    public QualificationMaxSpeedService(IRepositoryManager repositoryManager, IMapper mapper)
    {
        _repositoryManager = repositoryManager;
        _mapper = mapper;
    }

    public Task<IEnumerable<QualificationMaximumSpeedDto>> GetAll()
    {
        throw new NotImplementedException();
    }

    public async Task<QualificationMaximumSpeedDto> Create(
        QualificationMaximumSpeedForCreationDto raceResultItemForCreationDto)
    {
        ...
        var newQualificationResult = new QualificationMaximumSpeed
        {
            BookingId = items.Item3.Id,
            PositionId = items.Item5.Id,
            RaceweekId = items.Item6.Id,
            SectorId = items.Item8.Id,
            MaxAvgSpeed = raceResultItemForCreationDto.MaxAvgSpeed
        };

        _repositoryManager.QualificationMaximumSpeedRepository.Insert(newQualificationResult);
        await _repositoryManager.UnitOfWork.SaveChangesAsync();

        return _mapper.Map<QualificationMaximumSpeedDto>(newQualificationResult);
    }
