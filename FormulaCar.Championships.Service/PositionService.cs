...
public class PositionService : IPositionService
{
    ...
    {
        var position =
            (await _repositoryManager.PositionRepository.FindByCondition(x => x.Id == positionDto.Id,
                cancellationToken)).FirstOrDefault();
        if (position is null) throw new ItemNotFoundException(positionDto.Id);
    ...
    {
        var position =
            (await _repositoryManager.PositionRepository.FindByCondition(x => x.Id == positionId, cancellationToken))
            .FirstOrDefault();
        if (position is null) throw new ItemNotFoundException(positionId);
    ...
    public async Task<IEnumerable<PositionDto>> GetAll(CancellationToken cancellationToken = default)
    {
        ...
        if (positions is null) return new List<PositionDto>();
    ...
    {
        var position = (await _repositoryManager.PositionRepository.FindByCondition(x => x.Id == id, cancellationToken))
            .FirstOrDefault();

        if (position is null) throw new ItemNotFoundException(id);
