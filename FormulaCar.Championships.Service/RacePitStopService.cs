...
{
    public class RacePitStopService:IRacePitStopService
    {
        private readonly IMapper _mapper;
        private readonly IRepositoryManager _repositoryManager;

        public RacePitStopService(IRepositoryManager repositoryManager, IMapper mapper)
        {
            _repositoryManager = repositoryManager;
            _mapper = mapper;
        }
        public Task<IEnumerable<RacePitStopDto>> GetAll()
        {
        ...
        {
            (Driver, Season, Booking, Circuite, Position, Raceweek) items =
                await GetDetails(driver, circuit, season, position);

            if (items.Item5 == null || items.Item3 == null || items.Item6 == null)
            {
                return false;
            }

            return (await _repositoryManager.RacePitStop.FindByCondition(x
                    =>
                    x.BookingId == items.Item3.Id && x.RaceweekId == items.Item6.Id))
                .Any();
        }

        public async Task<RacePitStopDto> Create(RacePitStopForCreation raceFastestLapDto)
        {
            (Driver, Season, Booking, Circuite, Position, Raceweek) items =
                await GetDetails(raceFastestLapDto.Driver, raceFastestLapDto.Circuite, raceFastestLapDto.Season.ToString(), raceFastestLapDto.Postion);
            ...
            RacePitStop newPitStopItem = new RacePitStop()
            {
                TotalTime = raceFastestLapDto.TotalTime,
                Count = raceFastestLapDto.Count,
                BookingId = items.Item3.Id,
                PositionId = items.Item5.Id,
                RaceweekId = items.Item6.Id
            };

            _repositoryManager.RacePitStop.Insert(newPitStopItem);
            await _repositoryManager.UnitOfWork.SaveChangesAsync();

            return _mapper.Map<RacePitStopDto>(newPitStopItem);
        }


        async Task<(Driver, Season, Booking, Circuite, Position, Raceweek)> GetDetails(string driver, string circuit, string season,
          string position)
        {
